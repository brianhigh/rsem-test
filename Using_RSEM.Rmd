...
- Curl for getting fa, gtf, and isoforms files from Dropbox
...
SRA files have been downloaded: `r !cond_eval`, `r Sys.time()`, `r mem_used()`
...
for(i in 1:nrow(pd_small)) {
    ...
    if(file.exists(sshkey)) {
	    system(paste0('ascp -i ', sshkey ,' -k 1 -T -l200m ', aspera_url, 
                      " RSEM_test/SRA"))
    } else { 
...
SRA files are converted: `r !cond_eval`, `r Sys.time()`, `r mem_used()`
...
Reference genome ready: `r !cond_eval`, `r Sys.time()`, `r mem_used()`
...
RSEM results are recomputed: `r !cond_eval`, `r Sys.time()`, `r mem_used()`

```{r calculate-expression, engine='bash', eval=cond_eval, results='hide'}
# Run in this directory:
cd RSEM_test/RSEM
parallel -j 2 rsem-calculate-expression --bowtie2 -p 7 {} ../Reference_Genome/hg19 {/.} ::: ../FASTQ/*.fastq
```

We can now put all the RSEM counts/TPM results in one matrix, if it hasn't been done yet.

`r cond_eval <- length(list.files("RSEM_test/RSEM/", pattern="rsem_"))<3`
RSEM results are assembled: `r !cond_eval`, `r Sys.time()`, `r mem_used()`

```{r assemble-expression-matrix, eval=cond_eval}
...
rsem_tpm_matrix <- dcast(rsem_data_long, gene_id+transcript_ids~sample_name, value.var = "TPM", fill=0)
rsem_count_matrix <- dcast(rsem_data_long, gene_id+transcript_ids~sample_name, value.var = "expected_count", fill=0)
